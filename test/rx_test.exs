defmodule RxTest do
	...
	test "counting sources" do
		first = 1..10
		second = 11..20
		first_rx = first |> Rx.generate(50)
		second_rx = second |> Rx.generate(100)
		merged = Rx.merge(first_rx, second_rx)

		assert 0 == Reaxive.Rx.Impl.count_sources(first_rx)
		assert 0 == Reaxive.Rx.Impl.count_sources(second_rx)
		assert 2 == Reaxive.Rx.Impl.count_sources(merged)

		all = merged |>	Rx.take(15) # |> Rx.as_text
		assert 1 == Reaxive.Rx.Impl.count_sources(merged)

		#### Implementation idea:
		####
		#### store the disposables within the generators and
		#### send them togehter with the on_completed message
		#### (instead of nil). To, the disposable item can be used
		#### as to identify the source to be eliminated from
		#### the set of sources when a single source completes
		#### its sequence of events.



		assert Enum.concat(first, second) |> Enum.take(15) == (all |> Rx.stream |> Enum.sort)
	end

	test "a simple sequence generated by flat_map" do
		tens = Rx.return(10) |> Rx.flat_map(&(Rx.generate(1..&1))) |>
			Rx.stream |> Enum.to_list
		assert 1..10 |> Enum.to_list == tens
	end
