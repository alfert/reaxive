defmodule RxTest do
	...
	test "counting sources" do
		first_rx = 1..11 |> Rx.generate(20)
		second_rx = Rx.never()
		merger = fn(seq) -> Rx.merge(seq) |> Rx.as_text() |> Rx.take(1) end

		merged = Rx.merge(second_rx, Rx.return("single value"))# , second_rx)

		Logger.info("test process = #{inspect self}")
		Logger.info("first_rx  = #{inspect first_rx}")
		Logger.info("second_rx = #{inspect second_rx}")
		Logger.info("merged    = #{inspect merged}")
		Logger.info("merged.sources = #{inspect Reaxive.Rx.Impl.get_sources(merged)}")
		...
		all = merged |> Rx.as_text |>	Rx.take(1) # |> Rx.as_text
		Logger.info("all      = #{inspect all}")

		assert 1..10 |> Enum.to_list == (all |> Rx.stream |> Enum.sort)
